...
import { claimableMarketIds, nextPositionRefreshDelayMs, orderedUniqueMarketIds, playerPositionLabel, summarizePlayerPosition } from '@/lib/player-position';
...
describe('player position lifecycle', () => {
  ...
  it.each([
    [1, [1_000n, 0n, 0n, 0n] as const, 1_500n, 500n],
    [2, [0n, 1_000n, 0n, 0n] as const, 1_750n, 750n],
    [3, [0n, 0n, 1_000n, 0n] as const, 2_000n, 1_000n],
    [4, [0n, 0n, 0n, 1_000n] as const, 3_000n, 2_000n],
  ])('calculates the fixed-return win for outcome %i', (winner, stakes, payout, profit) => {
    expect(summarizePlayerPosition({ ...base, status: 4, winner, stakes })).toMatchObject({ payout, profit, lifecycle: 'claimable', action: 'claim' });
  });
  ...
  it('returns every stake when a round is cancelled', () => {
    ...
    expect(summary).toMatchObject({ stake: 3_000n, payout: 3_000n, profit: 0n, lifecycle: 'claimable', action: 'claim' });
  ...
  it('refreshes at the next on-chain deadline and keeps polling unresolved positions', () => {
    const awaiting = summarizePlayerPosition({ ...base, status: 1 });
    expect(awaiting.nextTransitionAt).toBe(201);
    expect(nextPositionRefreshDelayMs([awaiting], 199_500)).toBe(1_500);

    const recovery = summarizePlayerPosition({ ...base, status: 1, nowSeconds: 201 });
    expect(nextPositionRefreshDelayMs([recovery], 201_000)).toBe(30_000);
    expect(nextPositionRefreshDelayMs([
      summarizePlayerPosition({ ...base, status: 4, winner: 1 }),
    ], 201_000)).toBeNull();
  });

  it('selects only currently claimable markets for Claim all', () => {
    expect(claimableMarketIds([
      { marketId: 12n, lifecycle: 'claimable' },
      { marketId: 11n, lifecycle: 'awaiting_result' },
      { marketId: 10n, lifecycle: 'claimable' },
      { marketId: 9n, lifecycle: 'claimed' },
    ])).toEqual([12n, 10n]);
  });
});
