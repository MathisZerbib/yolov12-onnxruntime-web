...
import { Check, ChevronDown, CircleAlert, Copy, ExternalLink, Loader2, LogOut, RefreshCw, ShieldCheck, UserRound, Wallet, X } from 'lucide-react';
...
export function WalletButton() {
  ...
  useEffect(() => {
    ...
    fetch(`${AUTH_API_URL}/auth/session`, { credentials: 'include' })
      .then(response => setAuthState(response.ok ? 'authenticated' : 'idle'))
  ...
  return <div className="account-widget" ref={accountWidgetRef}>
    {!connected ? <button className="wallet-button" onClick={() => setWalletPickerOpen(true)}><Wallet /> Connect wallet</button> :
      <button className={`wallet-account-trigger ${authState === 'authenticated' ? 'verified' : ''}`} onClick={() => setAccountOpen(value => !value)}>
    ...
    {connected && accountOpen && <section className="wallet-account-panel">
